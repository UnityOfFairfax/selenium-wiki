...
| platform | string   | A key specifying which platform the browser should be running on. This value should be one of {`WINDOWS`, `XP`, `VISTA`, `MAC`, `LINUX`, `UNIX`, `ANDROID`}. When requesting a new session, the client may specify `ANY` to indicate any available platform may be used. For more information see [[GridPlatforms](GridPlatforms)] |
